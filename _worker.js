...
try{
  ...
  const px=tp.match(RE.P)?.[1]||P;
  const s5=tp.match(RE.S5)?.[1]||S5;
...
async function s5Conn(h,pt,cfg){
  ...
  try{
    ...
    const addr=s5Addr(h),req=new Uint8Array(3+addr.length+2);
    req[0]=5;req[1]=1;req[2]=0;req.set(addr,3);
  ...
  ts.readable.pipeTo(new WritableStream({
    async write(udp){
      try{
        ...
        const tid=setTimeout(()=>ac.abort(),TO);
        const resp=await fetch('https://1.1.1.1/dns-query',{method:'POST',headers:{'content-type':'application/dns-message'},body:udp,signal:ac.signal});
        clearTimeout(tid);
